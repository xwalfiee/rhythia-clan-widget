...
import { fetchClanStatistics } from "./services/rhythia.service";
...
async function initialize(env: unknown): Promise<void> {
	...
	try {
		...
		const statistics = await fetchClanStatistics(config);
...
export default {
	async fetch(
		...
		return new Response("Rhythia Clan Sync Worker Active", {
			status: 200,
